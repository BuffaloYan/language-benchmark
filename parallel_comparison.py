...
def main():
    parser = argparse.ArgumentParser(description='Parallel programming language comparison')
    ...
    sys_info = get_system_info()
    print(f"[SYSTEM] System: {sys_info['cpu_cores']} CPU cores, {sys_info['platform']}")
    print(f"[TEST] Test: {args.size:,} integers -> sort + prime counting")
    print()
    
    # Check data file
    if not os.path.exists(args.data_file):
        print(f"[DATA] Generating test data with {args.size:,} elements...")
        try:
            subprocess.run(['python3', 'python/generate_data.py', '--size', str(args.size), '--filename', args.data_file], check=True, encoding='utf-8', errors='replace')
        except FileNotFoundError:
            # Try with python if python3 doesn't exist
            subprocess.run(['python', 'python/generate_data.py', '--size', str(args.size), '--filename', args.data_file], check=True, encoding='utf-8', errors='replace')
